...
`)
    .action(async ({ session }, repo, eventsStr) => {
      if (!repo) return session?.text('commands.githubsth.subscribe.messages.specify_repo')
      if (!repoRegex.test(repo)) return session?.text('commands.githubsth.subscribe.messages.invalid_repo')
      ...
      if (trusted.length === 0) return session?.text('commands.githubsth.subscribe.messages.repo_not_trusted')
  ...
  ctx.command('githubsth.unsubscribe <repo>', '取消订阅 GitHub 仓库')
    ...
    .action(async ({ session }, repo) => {
      if (!repo) return session?.text('commands.githubsth.unsubscribe.messages.specify_repo')
      if (!session?.channelId) return session?.text('commands.githubsth.unsubscribe.messages.run_in_channel')
      ...
      if (result.matched === 0) return session?.text('commands.githubsth.unsubscribe.messages.not_found')
  ...
  ctx.command('githubsth.list', '查看当前频道订阅')
    ...
    .action(async ({ session }) => {
      if (!session?.channelId) return session?.text('commands.githubsth.list.messages.run_in_channel')
      const subs = await ctx.database.get('github_subscription', {
        channelId: session.channelId,
        platform: session.platform || 'unknown',
      })

      if (subs.length === 0) return session?.text('commands.githubsth.list.messages.empty')
      return subs
        .map((sub: any) => session?.text('commands.githubsth.list.messages.item', [
          sub.repo,
          sub.events.join(', '),
          sub.renderTheme || '默认',
          sub.renderStyle || '默认',
