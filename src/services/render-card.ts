...
const THEME_PRESETS: Record<RenderTheme, ThemePreset> = {
  'github-light': {
...
const THEME_ALIASES: Record<string, RenderTheme> = { 'gh-light': 'github-light', 'gh-dark': 'github-dark' }
const STYLE_ALIASES: Record<string, ThemeStyle> = { gh: 'github' }

let templateCache = ''

function getTemplate() {
  if (!templateCache) {
    const templatePath = join(__dirname, '..', 'assets', 'render-card.html')
...
function pickCommitIcon(message: string) {
  const lower = (message || '').toLowerCase()
  if (lower.startsWith('feat')) return '[F]'
  if (lower.startsWith('fix')) return '[X]'
  if (lower.startsWith('docs')) return '[D]'
  if (lower.startsWith('refactor')) return '[R]'
  if (lower.startsWith('test')) return '[T]'
  if (lower.startsWith('chore')) return '[C]'
  return '[*]'
}
...
function getEventTitle(event: string, payload: any) {
  switch (event) {
...
function getCardType(event: string, payload: any) {
  if (event === 'digest') return 'DIGEST'
  if (event === 'pull_request' || event === 'pull_request_review') return 'PR'
  if (event === 'issues' || event === 'issue_comment') return 'ISSUE'
  if (event === 'push') return 'COMMIT'
  if (event === 'release') return 'RELEASE'
  return 'REPO'
}

function resolveTone(event: string, payload: any): Tone {
  ...
  if (result === 'success' || result === 'approved' || action === 'merged' || state === 'closed') return 'success'
  if (result === 'failure' || result === 'changes_requested') return 'danger'
  if (action === 'opened' || action === 'created' || action === 'published') return 'info'
  if (action === 'edited' || action === 'reopened') return 'warning'
  return 'neutral'
}

function pill(text: string, tone: Tone) {
  const color = getToneColor(tone)
  return `<span class="pill" style="border-color:${color};color:${color};background:${color}22;">${escapeHtml(text)}</span>`
}

function buildStatusPills(event: string, payload: any) {
  const pills: string[] = []
  if (event === 'pull_request' && payload?.pull_request?.state) pills.push(pill(String(payload.pull_request.state), resolveTone(event, payload)))
  if (event === 'workflow_run' && payload?.workflow_run?.conclusion) pills.push(pill(String(payload.workflow_run.conclusion), resolveTone(event, payload)))
  if (event === 'release' && payload?.release?.tag_name) pills.push(pill(String(payload.release.tag_name), 'info'))
  return pills.join('')
}
...
function buildDetailBlock(event: string, payload: any) {
...
function buildCommitBlock(event: string, payload: any) {
  if (event !== 'push' || !Array.isArray(payload?.commits) || payload.commits.length === 0) return ''
  const rows = payload.commits.slice(0, 8).map((commit: any) => {
...
function buildDigestBlock(payload: any) {
  const items = Array.isArray(payload?.__digestItems) ? payload.__digestItems : []
  if (!items.length) return ''
  const rows = items.slice(0, 30).map((item: any) => {
...
export function listRenderThemes() {
  return Object.keys(THEME_PRESETS) as RenderTheme[]
}

export function listRenderStyles() {
  return ['auto', 'github', 'glass', 'neon', 'compact', 'card', 'terminal'] as RenderStyle[]
}

export function getThemeDefaultStyle(theme: RenderTheme): ThemeStyle {
  return (THEME_PRESETS[theme] || THEME_PRESETS['github-dark']).style
}

export function normalizeRenderTheme(theme?: string | null): RenderTheme | null {
...
export function normalizeRenderStyle(style?: string | null): RenderStyle | null {
...
export function buildRenderHtml(textMessage: string, event: string, payload: any, theme: RenderTheme, width: number, style?: RenderStyle | null) {
  const preset = THEME_PRESETS[theme] || THEME_PRESETS['github-dark']
  const resolvedStyle = (style && style !== 'auto' ? style : preset.style) as ThemeStyle
