...
export class Formatter extends Service {
  ...
  formatPush(payload: any) {
    ...
    const preview = commits.slice(0, 5).map((commit: any) => {
  ...
  private summarizeCommentBody(raw: string) {
    const content = String(raw || '').replace(/\r/g, '').trim()
    if (!content) return this.t('emptyComment')
    ...
    if (looksEncoded) return this.t('hiddenMachinePayload')
  ...
  private t(key: keyof typeof I18N['zh-CN'], params: Array<string | number> = []) {
    const template = I18N[this.locale][key] as string
